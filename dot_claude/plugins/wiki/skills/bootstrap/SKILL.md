---
name: wiki:bootstrap
description: Initialize a personal LLM wiki for an Obsidian vault. Creates ~/my-wiki/config.json and generates wiki/schema.md inside the vault with conventions for frontmatter, cross-references, index, and log. Use when the user mentions "wiki bootstrap", "wiki 初期化", "wiki セットアップ", "personal wiki を始めたい", or when the user asks to start a new personal knowledge base maintained by Claude. Always run this before wiki:ingest/query/lint if the config or schema does not exist yet — the other skills depend on it.
---
...
1. `<vault>/wiki/config.json` — vault 内設定 (vault 自身が SSOT、外部にパスを持たない)
2. `<vault>/wiki/schema.md` — wiki 運用規約 (frontmatter / cross-ref / index / log 等)
...
2. **`wiki/` サブディレクトリ** — vault 内 `wiki/` 直下を wiki ルートとする。既存 `wiki/` があれば規約をそこから読み取り、schema.md に逆輸入する (破壊禁止)。
3. **初期カテゴリ** — `raw/` のトップレベルディレクトリ名を初期カテゴリ候補として提示。ユーザーが採用を選んだら `<vault>/wiki/<category>/` を空ディレクトリで作る (`.gitkeep` 置いて ok)。bootstrap 後は wiki 下の subdir がカテゴリの実体なので、以降 config に記録は不要。

## config.json

`<vault>/wiki/config.json` に書く:
...
- 他 skill は `<vault>/wiki/config.json` を読み、`ls <vault>/<wiki_dir>/` で現在のカテゴリ一覧を取得する

## schema.md

`<vault>/wiki/schema.md` を生成する。これは wiki 運用規約を LLM 自身が読み返すための設計書。既存 vault に規約があるなら踏襲、無ければ以下のデフォルトを書き込む:
...
- 画像は `<vault>/raw/assets/` を参照

### index.md

`<vault>/wiki/index.md` はカテゴリ別の目次。以下の形式:
...
`<vault>/wiki/log.md` は append-only の時系列記録。形式:
...
- カテゴリ名は raw/wiki 間で一致させる
- wiki のファイル名は `<title>.md` (日本語可)
- **`raw/assets/` の用途**: 画像 (png/jpg/gif/webp)、PDF、その他バイナリ添付。カテゴリ跨ぎで参照されるものなのでフラットに置く
- wiki ページから assets を参照する時は wikilink `[[raw/assets/<file>]]` (画像なら `![[raw/assets/<file>.png]]` で埋め込み)
- Obsidian 側の添付パス設定 (`Settings → Files and links → Attachment folder path`) を `raw/assets` にしておくと、Obsidian Web Clipper の画像ダウンロードもここに集まる

## bootstrap の手順

1. vault パスをユーザーから受け取る (絶対パス、iCloud 等の空白含みもOK)
2. `<vault>/wiki/config.json` の既存有無を確認。あれば上書き可否を聞く
3. wiki_dir / raw_dir (既定値で十分) を決定
4. `raw/` のトップレベル subdir から初期カテゴリ候補を提示、採用分を `<vault>/<wiki_dir>/<category>/` に空作成
5. `<vault>/wiki/config.json` を書き込む
6. `<vault>/<wiki_dir>/schema.md` を生成 (既存規約があれば反映)
7. `wiki/index.md` と `wiki/log.md` が無ければ空のものを作成 (あれば触らない)
...
- `sources` の書き方 (`[[raw/<file>]]` flat vs `[[raw/<category>/<file>]]` 階層)
