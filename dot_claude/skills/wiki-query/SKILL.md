...
filing に同意が得られたら `wiki:ingest` と同じ書式で新ページを作る。`category` は配置するディレクトリに応じて `wiki-<ディレクトリ名>`。frontmatter の `sources` は参照した wiki ページを wikilink で列挙 (raw でなく wiki を指す点で ingest と異なる)。log.md の先頭に prepend:
...
- wiki 外の情報で wiki ページを無断追加しない (それは ingest の仕事)
